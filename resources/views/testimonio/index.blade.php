...
<div class="card-header bg-primary ">
  <div class="color-palette">
    <h1 class="text-center"><strong>Testimonios</strong></h1>
  </div>
</div>
  ...
  <div class="card-body text-dark">
    <table id="example1" class="table table-striped table-bordered nowrap"  style="width:100%">
      ...
      <tbody>
        ...
        <tr>
          <td>{{ $testimonios->id }}</td>
          <td>{{ $testimonios->proyectos->id }}</td>
          <td>{{ $testimonios->comunidades->comunidad }}</td>
          <td>{{ $testimonios->testimonio }}</td>
          <td>
          <a class="btn btn-primary" href="#
          {{-- {{ route('comentario.show', $comentario->id ) }} --}}
          ">Mostrar</a>
          </td>
        </tr>
        @endforeach
